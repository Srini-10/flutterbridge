...
- **imperative navigation** — `Navigator.push`, `pushNamed`, `pop`. The analyzer already emits the edge,
  the route table is generated and the runtime router is live; what no UIR node says is *perform this
  transition here*. [ADR-0025](adr/0025-the-navigation-model.md) D2 names the construct.
- **route overlays** — `showDialog`, `showModalBottomSheet`, `showMenu`. The *same* amendment: each pushes
  a `Route`, so an overlay is a navigation to an inline destination.
- **messenger overlays** — `showSnackBar`, `ScaffoldMessenger`. A **different** blocker, despite reading
  alike: a snack bar is not a route, it is enqueued on the nearest `ScaffoldMessenger`, which owns its
  lifetime and dismissal. No ADR models that yet. A `MaterialBanner` *is* in the tree and renders.
...
None of these is a defect in your Flutter code, and none has a source-level workaround beyond the ones
named above. If a navigation diagnostic ever reads as though your program is at fault, that is a bug in the
diagnostic — [M6-E](m6/m6e-navigation-diagnostics-audit.md) fixed three of them.

## `BRG3008` — a navigation constructs its destination inline

`Navigator.push(context, MaterialPageRoute(builder: (_) => Detail(id: 7)))` names no route, so it has no
path. Two things are missing, both owned by the compiler: the node that performs the transition
([ADR-0025](adr/0025-the-navigation-model.md) D2), and a URL for a destination that never had one — which
this generator will not invent, because inventing `/_push/Detail` is a routing policy you did not write.

**Workaround**: declare the destination in `routes:` and navigate to it by name. That gives it a real path
you chose. The push itself is still refused until D2 lands, but the route becomes visible to the router and
to cross-route state promotion.

## `BRG3018` — a route's component needs constructor arguments

`home: CounterPanel(label: 'Taps')` where `CounterPanel` requires `label`. The arguments **are** extracted —
they are the `props` of the element in the app root's slot — but `app.Route` has no field linking a route to
them, so the emitted page would call the component with nothing.

Before this diagnostic existed the build reported *success* and emitted a project that failed `tsc`.
[ADR-0025](adr/0025-the-navigation-model.md) D1 closes it.

**Workaround**: give the route's component defaults for its required parameters, or read the values from a
store instead of passing them in.
