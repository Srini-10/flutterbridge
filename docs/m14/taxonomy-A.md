...
Normalizer errors: **0**. Generator errors (past the normalizer): **362**. Unique root causes: **19**.

Occurrences by category — A (contract bug): 0; B (browser-compatible, not implemented): 169; C (library adapter): 192; D (no browser equivalent): 1; unclassified: 0.

| Count | Stage | Cat | Root cause | First failing layer | Codes | ADR |
| ---: | --- | --- | --- | --- | --- | --- |
| 70 | generate | C | a name with no declaration in the program (usually a package or a private symbol from a construct above) | generator | BRG3006 | 0073 |
| 50 | generate | C | named arguments to a package/SDK callee the kit does not mirror (Supabase, showModalBottomSheet, Color ops, …) | generator (no callee signature for a package/SDK function) | BRG3002 | 0073 |
| 49 | generate | B | a theme built by a helper/ColorScheme from computed values: no token for the Material role a widget paints | analyzer (token extraction) | BRG3010 | 0073 |
...
| 25 | generate | B | an expression form with no lowering | generator | BRG3002 | 0058 |
...
| 15 | generate | C | Riverpod: ref.watch/read/listen, providers, notifiers | analyzer (no model of ConsumerWidget/ref) | BRG3013 | 0073 |
...
| 11 | generate | C | member of a package class with no runtime model (dio beyond the subset, audio, Supabase, …) | generator | BRG3013 | 0073 |
| 11 | generate | B | a construct the generator names as unsupported | generator | BRG3013 | 0073 |
