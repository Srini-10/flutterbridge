...
*The figures in the two bullets below are the M14 burn-down snapshot (`taxonomy-A.md`, `taxonomy-B.md`). Later Riverpod phases each committed their own snapshot beside them (`taxonomy-*-riverpod-watch`, `-notifier`, `-asyncvalue`, `-builder-locals`); the latest, `taxonomy-A-sdk-lowering.md` / `taxonomy-B-sdk-lowering.md` (after `StringBuffer`, `int`/`double.toString` and the numeric rounding methods — `sdk-text-and-numeric-lowering.md`), has App A at **346** generator errors and App B at **4 489**, neither generating successfully (`taxonomy-*-builder-locals` was 348 / 4 653). Per-construct accounting is in `riverpod-usage-matrix.md` §4a–§4k — the aggregate moves both ways as code that used to fail as one opaque blob is reached and reported.*
