...
`bridge build` stages: analyze FAILED.
Normalizer errors: **0**. Generator errors (past the normalizer): **4506**. Unique root causes: **24**.

Occurrences by category — A (contract bug): 0; B (browser-compatible, not implemented): 1577; C (library adapter): 2899; D (no browser equivalent): 25; unclassified: 5.

| Count | Stage | Cat | Root cause | First failing layer | Codes | ADR |
| ---: | --- | --- | --- | --- | --- | --- |
| 857 | generate | C | Riverpod: ref.watch/read/listen, providers, notifiers | analyzer (no model of ConsumerWidget/ref) | BRG3013 | 0073 |
| 651 | generate | C | a name with no declaration in the program (usually a package or a private symbol from a construct above) | generator | BRG3006, BRG3013 | 0073 |
