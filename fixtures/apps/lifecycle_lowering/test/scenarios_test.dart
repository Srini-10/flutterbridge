...
import 'package:lifecycle_lowering/main.dart';
...
// (`packages/generators/react/tests/lifecycle_lowering_execution.test.ts`).
//
...
final Map<String, Widget Function()> widgets = <String, Widget Function()>{
  'LifecycleHost': () => const LifecycleHost(),
  'UpdateHost': () => const UpdateHost(),
  'OrderHost': () => const OrderHost(),
  'AsyncHost': () => const AsyncHost(),
  'InitOnly': () => const InitOnly(),
};
...
void main() {
  final Map<String, dynamic> scripts =
      jsonDecode(File('test/scenarios.json').readAsStringSync())
    ...
    testWidgets('scenario ${entry.key}', (WidgetTester tester) async {
      ...
      for (final Object? label in entry.value as List<dynamic>) {
        if (label == '@wait') {
          ...
          await tester
              .tap(find.widgetWithText(ElevatedButton, label! as String));
          await tester.pump();
        }
