...
import 'package:layout/layouts.dart';

// The oracle for the generated React components: each scenario names a widget hosting LayoutBuilders and the buttons that
// resize the box around them; after mounting and every tap it records the `Text`s starting with "> ". The generated component
// is driven through the same script in jsdom (with a layout shim) and must show the same texts
// (`tests/layout_execution.test.ts`).
//
// `WRITE_EXPECTED=1 flutter test` rewrites `expected.json`.

final Map<String, Widget Function()> widgets = <String, Widget Function()>{
  'Widths': () => const Widths(),
  'Responsive': () => const Responsive(),
